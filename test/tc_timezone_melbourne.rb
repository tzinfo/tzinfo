...
class TCTimezoneMelbourne < Minitest::Test
  def test_2004
    ...
    time_types_test do |h|
      ...
      assert_equal(39600, tz.period_for(h.time(2004, 3,27,15,59,59,0,0)).utc_total_offset)
      assert_equal(36000, tz.period_for(h.time(2004, 3,27,16, 0, 0,0,0)).utc_total_offset)
      assert_equal(36000, tz.period_for(h.time(2004,10,30,15,59,59,0,0)).utc_total_offset)
      assert_equal(39600, tz.period_for(h.time(2004,10,30,16, 0, 0,0,0)).utc_total_offset)

      assert_equal(39600, tz.period_for_utc(h.time(2004, 3,27,15,59,59)).utc_total_offset)
      assert_equal(36000, tz.period_for_utc(h.time(2004, 3,27,16, 0, 0)).utc_total_offset)
      assert_equal(36000, tz.period_for_utc(h.time(2004,10,30,15,59,59)).utc_total_offset)
      assert_equal(39600, tz.period_for_utc(h.time(2004,10,30,16, 0, 0)).utc_total_offset)

      assert_equal(39600, tz.period_for_local(h.time(2004, 3,28,2,59,59),  true).utc_total_offset)
      assert_equal(36000, tz.period_for_local(h.time(2004, 3,28,2,59,59), false).utc_total_offset)
      assert_equal(39600, tz.period_for_local(h.time(2004, 3,28,2, 0, 0),  true).utc_total_offset)
      assert_equal(36000, tz.period_for_local(h.time(2004, 3,28,2, 0, 0), false).utc_total_offset)
      assert_equal(36000, tz.period_for_local(h.time(2004,10,31,1,59,59)).utc_total_offset)
      assert_equal(39600, tz.period_for_local(h.time(2004,10,31,3, 0, 0)).utc_total_offset)
  ...
  def test_1942
    ...
    time_types_test do |h|
      ...
      assert_equal(39600, tz.period_for(h.time(1942,3,28,14,59,59,0,0)).utc_total_offset)
      assert_equal(36000, tz.period_for(h.time(1942,3,28,15, 0, 0,0,0)).utc_total_offset)
      assert_equal(36000, tz.period_for(h.time(1942,9,26,15,59,59,0,0)).utc_total_offset)
      assert_equal(39600, tz.period_for(h.time(1942,9,26,16, 0, 0,0,0)).utc_total_offset)

      assert_equal(39600, tz.period_for_utc(h.time(1942,3,28,14,59,59)).utc_total_offset)
      assert_equal(36000, tz.period_for_utc(h.time(1942,3,28,15, 0, 0)).utc_total_offset)
      assert_equal(36000, tz.period_for_utc(h.time(1942,9,26,15,59,59)).utc_total_offset)
      assert_equal(39600, tz.period_for_utc(h.time(1942,9,26,16, 0, 0)).utc_total_offset)

      assert_equal(39600, tz.period_for_local(h.time(1942,3,29,1,59,59),  true).utc_total_offset)
      assert_equal(36000, tz.period_for_local(h.time(1942,3,29,1,59,59), false).utc_total_offset)
      assert_equal(39600, tz.period_for_local(h.time(1942,3,29,1, 0, 0),  true).utc_total_offset)
      assert_equal(36000, tz.period_for_local(h.time(1942,3,29,1, 0, 0), false).utc_total_offset)
      assert_equal(36000, tz.period_for_local(h.time(1942,9,27,1,59,59)).utc_total_offset)
      assert_equal(39600, tz.period_for_local(h.time(1942,9,27,3, 0, 0)).utc_total_offset)
